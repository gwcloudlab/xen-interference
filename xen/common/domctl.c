...
long do_domctl(XEN_GUEST_HANDLE(xen_domctl_t) u_domctl)
{
    ...
    { 
		...
		op->u.getvcpuinfo.running_time = runstate.time[RUNSTATE_running];
		...
		op->u.getvcpuinfo.num_boost_schedule = ((struct csched_vcpu*)(v->sched_priv))->num_pri_schedule.batch_schedule;
		op->u.getvcpuinfo.num_batch_schedule = ((struct csched_vcpu*)(v->sched_priv))->num_pri_schedule.boost_schedule;
		...
		op->u.getvcpuinfo.num_boost = ((struct csched_vcpu*)(v->sched_priv))->num_pri.batch;
		op->u.getvcpuinfo.num_batch = ((struct csched_vcpu*)(v->sched_priv))->num_pri.boost;

        printk("Printing Stats for %d. batch_schedule=%llu\n", v->vcpu_id,  (unsigned long long)((struct csched_vcpu*)(v->sched_priv))->num_pri_schedule.batch_schedule);
